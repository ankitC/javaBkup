...
public class FaceRec{
	...
	/*Modifications to the code by macro _hyrax*/
		...
		try{
			...
			for(int i=0;i<distances.length;i++){
				...
				if (minimumDistance < thresholdVal){
					...
					Path localFSPath=new Path(matchingFileName);
					
					String filename = matchingFileName.substring(matchingFileName.lastIndexOf('/') + 1,
							matchingFileName.length());
					String hdfsOutputFile="/output/"+num+"_"+filename;
					Path hdfsOutputPath=new Path(hdfsOutputFile);
					hdfsFS.copyFromLocalFile(localFSPath, hdfsOutputPath);
	...
	public Map<String, Double> processSelections(String faceImageName,String directory,String numofFaces,String threshold) {
	...
	//demo main
	public static String recognize(String testImagePath){
		long start = System.currentTimeMillis();
		
		Path hdfsSrc=new Path(testImagePath);
		
		 String filename = testImagePath.substring(testImagePath.lastIndexOf('/') + 1,
		           testImagePath.length());
		String localImage="/home/ubuntu/faceReco/hyraxv2/subjectImage/"+filename;
		Path lfsImagePath=new Path(localImage);
		
		JobConf conf=new JobConf();
		
		try {
			FileSystem fsys=FileSystem.get(conf);
			fsys.copyToLocalFile(hdfsSrc, lfsImagePath);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
				
		String imgToCheck =localImage;
		String imgDir = "/home/ubuntu/faceReco/hyraxv2/testFolder";
