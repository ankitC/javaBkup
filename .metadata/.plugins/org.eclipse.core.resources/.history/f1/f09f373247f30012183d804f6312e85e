...
public class FaceRec{
	...
	public Map<String, Double> resultDistances=new HashMap<String, Double>(); 
	...
	/*Returns results as filenames*/
	public Map<String, Double> findMatchResult(String imageFileName,int selectedeigenfaces,double thresholdVal){
		...
		try{
			...
			for(int i=0;i<distances.length;i++){
				ImageDistanceInfo distanceInfo =new ImageDistanceInfo(distances[i], i);
				...
				if (minimumDistance < thresholdVal){
					...
					Path matchingFilePath= new Path(matchingFileName.toString());
					String filePath = matchingFileName.substring(matchingFileName.lastIndexOf('/') + 1,
							matchingFileName.length());
					org.apache.hadoop.fs.FileSystem filesys=org.apache.hadoop.fs.FileSystem.get(config);
					String out="/output/"+filePath;
					Path outputFile=new Path(out);
					System.out.println(matchingFilePath);
					System.out.println(out);
					filesys.copyFromLocalFile(matchingFilePath, outputFile);
					resultDistances.put(matchingFileName, minimumDistance);
				}
			}
			System.out.println(resultDistances);
	...
	private void doCalculations(String dir,List<String> imglist,int selectedNumOfEigenFaces) throws FaceRecError, IOException{
		...
		this.bundle = b;
		this.weights = wts;
		System.out.println("Going to write Cache...");
		writeCache(dir,b);
	...
	private void validateSelectedImageFileName(String faceimagename) throws FaceRecError{
		System.out.println("FaceImageName is:"+faceimagename+" and length is:"+faceimagename.length());
		if ( (faceimagename.length() == 0) || (!isImage(faceimagename)) ){
			System.out.println("Is not an image file");
	...
	private boolean isImage(String imagefilename){
		...
		/*try{
			...
			isimage = false;
			System.out.println("Could not read file at:"+imagefilename);
		}*/
		return isimage; 
	}
	...
	public Map<String, Double> processSelections(String faceImageName,String directory,String numofFaces,Double threshold) {
		System.out.println("In Process Selections");
		System.out.println(faceImageName);
		Map<String, Double> result = null;
		int numFaces = 0;
		double thresholdVal = threshold;
		try{
			System.out.println("Checking the image file");
			validateSelectedImageFileName(faceImageName);
			System.out.println("Validated Image");
			validateSelectedFolderName(directory);
			System.out.println("Validated Folder");
			numFaces = getNumofFacesVal(numofFaces);
			System.out.println("Valid number of faces:"+numFaces);
			...
			System.out.println("Cache Check done!");
			System.out.println("Reconstructing faces...\n");
			reconstructFaces(numFaces);
			System.out.println("Entering Match Result");
			result = findMatchResult(faceImageName,numFaces,thresholdVal);
		}catch(Exception e){
			System.out.println("Exception in Process Selections:");
			result = resultDistances;
		}
		return result;
	}

	/*Main function of the App*/
	public static String recognize(String subjectPath){
		...
		String thresholdVal = args[3];*/
		 ...
		 org.apache.hadoop.fs.FileSystem fileSystem = org.apache.hadoop.fs.FileSystem.get(conf);
		    ...
		    Path path = new Path(subjectPath);
		    if (!fileSystem.exists(path)) {
		        System.out.println("File does not exists");
		        System.exit(0);
		    }
		    /*Read the image from HDFS to LFS*/
		    FSDataInputStream in = fileSystem.open(path);
		    String filePath = subjectPath.substring(subjectPath.lastIndexOf('/') + 1,
		            subjectPath.length());
		    String filename="/usr/local/hyraxv2/subjectImage/"+filePath;
		...
		String imgToCheck =filename;
		String imgDir = "/usr/local/hyraxv2/testFolder";
		
		String numFaces = "12";
		Double thresholdVal = 0.8;
		
		/*Finding the matching files*/
		System.out.println("ENtering Process Selections");
		Map<String, Double> result = new FaceRec().processSelections(imgToCheck,imgDir,numFaces,thresholdVal);
		System.out.println("Finished Process Selections");
		Path outputDir=new Path("/output");
	/*if (!result.isEmpty()){
			//Upload the matching files to HDFS to the output dir specified
			Path outputDir=new Path("/output");
		    ...
		    while(iterator.hasNext()) {
		       ...
		       Path sourcePath=new Path(setElement);
		      //  WriteToHDFS.addFile(fileSystem, setElement, outputDir);
		       fileSystem.copyFromLocalFile(sourcePath, outputDir);
		    }*/
		    
		   returnPath=outputDir.toString();
